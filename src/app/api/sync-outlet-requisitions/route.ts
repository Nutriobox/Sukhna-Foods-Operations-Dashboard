...
/**
 * Queues a NutrioBox pending-requisition sync for the AWS worker
 * (public.pact_jobs, payload.__sync = "outlet-requisitions"). The worker runs
 * scripts/sync-outlet-requisitions.js and refreshes public.pending_outlet_requisitions.
 */
...
export async function POST() {
  ...
  try {
    const r = await fetch(`${sbUrl.replace(/\/$/, "")}/rest/v1/pact_jobs`, {
      ...
      body: JSON.stringify({ id: jobId, invoice: "sync:outlet-requisitions", status: "queued", dry_run: false, payload: { __sync: "outlet-requisitions" } }),
    });
