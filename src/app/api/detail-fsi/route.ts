...
export async function GET(req: Request) {
  ...
  if (inv) {
    ...
    try {
      const r = await fetch(`${url0}/rest/v1/invoice_lines?kind=eq.b2b&doc_no=eq.${encodeURIComponent(inv)}&select=cols,lines`, { headers: { apikey: key0, Authorization: `Bearer ${key0}` }, cache: "no-store" });
